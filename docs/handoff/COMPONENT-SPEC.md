...
| **CodecManager**      | `features/settings/CodecManager`          | CodecRow ×n                                                                                           | No props — a **Codec report**, not a manager: the organism owns `useCapabilities` and draws the **Codec summary** (`N formats enabled · M from the playback component`, the copy amended in `5db8da0` — the rows it counts are what the bundled component decodes, which the family never added) over one `CodecRow` `{ row: CodecRowModel }` per catalogued codec the report contains, in **Format catalogue** order. Each row: the tile with `MicrochipIcon`, the display name, the **Container chips**, a `—` where the prototype's size cell is, the **Status pill** (`Built-in` / `Installed`), and the 32px spacer where a ✕ would sit. No _Add a codec pack_ drop zone and no per-row ✕ until the **Playback component upload** initiative; the zone, when it comes, takes a **playback component** (the FFmpeg binary), not a `.dll` — no uploaded library changes what a browser decodes, so the copy must not promise it. The spec's `{ summaryLabel, codecs, onBrowse }` collapsed the way `ExportModal`'s did — the organism reads the wire itself. Blank until the report lands, and blank still on a refused read.                                                                                                                                         |
