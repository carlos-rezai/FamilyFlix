...
| **PlayerControls** | `features/player/PlayerControls`     | ProgressBar (scrubber), IconButton                                            | `{ playing, currentTime, duration, volume, muted, subsOn, controlsVisible, showBigPlay, showBuffering, showUnavailable, noticeTitle, noticeBody, + handlers }` — the centre of the picture is one element, the 96px circle, drawn as **big-play**, as **buffering** (spinner + caption), or as **unavailable** (`missing-file` and `cannot-play`, told apart by `noticeTitle`/`noticeBody`), and absent while the film runs. No second centred element. The Back pill never idles away behind a notice, so neither message is a trap. |
