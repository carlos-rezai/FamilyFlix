# Refactor plan: Delete a movie — one containment rule, two radii, and the tests read by behaviour
...
nothing wrong, three things written twice or written in build order.
...
### 2. Two radii the tokens already have
...
the server fold first because it is the one with a real duplicate, the radii
second because they are two lines, the test regroup last because it moves the
most text and changes no assertion.
...
1. **`removeFolder` asks `containedFolder`.** Replace the inline resolve-and-
   ...
   folder it was given; a `null` answer is the same early return it makes
   today. The `rmSync` stays as it is and keeps throwing — `removeFolder` is a
   rollback, before any commit, and the log's rule that it does not swallow is
   unchanged. `containedFolder`'s docblock loses "asked of a folder rather than
   a file" as the thing that distinguishes it and gains the two callers. All
   five `removeFolder` tests and all six `removeMovieFolder` tests unchanged.

2. **`containedFolder` sits with `realRoot`.** Move the helper up to follow
   `realRoot` directly, so the three folder-side helpers read in dependency
   order: the root, a folder inside it, the free name under it. A move, not an
   edit.

### Group 2 — two radii

3. **The icon tile's corner is `radius.md`.** `Modal.styles.ts`, `IconTile`:
   `border-radius: ${({ theme }) => theme.radius.md}`. Same pixel.

4. **The ✕'s corner is `radius.sm`.** `Modal.styles.ts`, `CloseButton`:
...
5. **The heading test joins the copy.** In `DeleteMovieDialog.test.tsx`, the
...
6. **Confirming is one block.** "afterwards", "in flight", "dismissed
...
7. **One `DELETE` block.** In `routes.test.ts`, the "— the bytes" block's tests
...
- Group 2 has no automated test, on the same footing as every other pixel
  ...
  not a behaviour. Checked in the browser.
...
- **`FilterDropdown`'s `999px`** is `radius.pill` written out, in a file the
  search-filter round owns. Same rule, different initiative; noted, not taken.
