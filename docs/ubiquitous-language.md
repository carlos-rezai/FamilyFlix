...
| **Optimistic save** (updated) | The bargain an edit keeps with the server: show the new value at once, take the route's echo over what was assumed, put back what the edit cost if the save is refused. Written twice, for two shapes. `useOptimisticSave` reverts by negating a flag and addresses a **Movie** by id inside a list — the two browse screens' **Favorite** hearts. `useOptimisticEdit` is told what to put back rather than deriving it, and edits the one **Movie** a page is holding — all three of the **Movie detail page**'s writes, the watched tick, its own heart and the **Rating picker**. A **Rating** is why the second one exists: eleven values plus an absence, where `!value` can express none of it. | optimistic update, local write, cache   |
