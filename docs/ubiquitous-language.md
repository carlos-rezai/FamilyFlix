...
| **Playback component** (updated) | The resolved FFmpeg binary — `FAMILYFLIX_FFMPEG_PATH`, then `PATH`, then **absent** — that makes **Remux** and **Transcode** possible. Bundled by the installer and replaceable by the maintainer, both of which fill the same first slot. This is what the prototype's "codec pack" actually is. It is the app's **one pack**: every **Installed** row of the **Codec report** comes and goes with it, which is why no row has a size or a remove of its own (`15-settings-hub` Q7). Replacing it is the **Playback component upload**, a later initiative.                                               | codec pack, codec, plugin, ffmpeg     |
...
| **Codec report** (new)                | `feat.CodecManager` → `features/settings/CodecManager`: the **Codec summary** over one **Codec row** per **Format row model**, read from `GET /api/playback/capabilities`. A report, not a manager: nothing on it installs or removes anything until the **Playback component upload** ships.                                                                                           | codec manager, codec list, formats panel     |
| **Codec row** (new)                   | One format the machine decodes — the microchip tile, the display name, its **Container chips**, a `—` where the prototype's size cell is, and a **Status pill**. `features/settings/CodecRow`. No ✕: a codec cannot be removed on its own.                                                                                                                                              | codec item, format line                      |
...
| **Status pill** (new)                 | **Built-in** for a codec Chromium decodes (`native`), **Installed** for one the **Playback component** decodes (`via-component`) — the two words the prototype has, true of a bundled and an uploaded component alike.                                                                                                                                                                  | badge, tag, support label                    |
...
| **Playback component upload** (new)   | The later initiative that draws the prototype's dashed _Add a codec pack_ zone: a writable slot under user-data ahead of the installer's, a component swappable while the app runs, a two-file upload validated by running it, and the component's own size and remove. Not this hub's; the zone is not drawn until it ships.                                                           | codec pack upload, add codec, ffmpeg upload  |
...
- The **Settings hub** draws four **Settings groups**; the Library group is **Action rows**, the other three are one **Section card** each. A control whose mechanism does not exist — _Add a codec pack_, _Change…_, _Software update_ — is not drawn, the way the **Export** row was not drawn until its dialog existed; only a control the prototype itself marks **Coming soon** is drawn disabled.
...
- Every **Installed** row exists because of the one **Playback component**; none has a size or a remove, and all of them change together when the **Playback component upload** replaces it.
