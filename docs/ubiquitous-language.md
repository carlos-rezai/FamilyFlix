...
| **Series page** (new)           | `/series/:id` — the movie page's hero shape for a **Series**: poster, **Year range**, season and episode count, read-only stars, genres, _Resume S02E04_, the heart, the progress line, synopsis, _Created by_ / _Starring_, over the **Season cards**. No ⋯ menu, no rating picker, no watched toggle. | series detail, show page                      |
...
| **Playable** (new)              | What the **Player** is handed in place of a movie id: `{ kind: 'movie' \| 'episode'; id }`, choosing which of two parallel wires (`/api/movies/:id/*`, `/api/episodes/:id/*`) its reads and writes run over. One player, one set of rules, two kinds of file.                                           | media item, video, source, track              |
...
| **Episode tag** (new)           | The `S01E03` or `1x03` in an episode's filename — `episodeTag`, in `media/`, reads the season, the episode (the first of a multi-episode file) and the title after it. The only source of an episode number.                                                                                            | episode code, SxxEyy, marker                  |
...
- **`PosterCardMovie` holds a Series (new):** the **Series tab**'s grid feeds
  ...
  carries; renaming it is a refactor for when a third kind arrives, not now.
