...
| **Optimistic save** (updated) | The bargain an edit keeps with the server: show the new value at once, take the route's echo over what was assumed, put the old value back if the save is refused. `useOptimisticSave` in code holds the **boolean** case, and is used by the two browse screens' **Favorite** hearts only; `useMovieDetail` hand-rolls **all three** of its writes — the watched tick, its own heart and the **Rating picker** — because a **Rating** has eleven values plus an absence and must be _told_ what to put back. That third copy is filed as `useOptimisticEdit`. | optimistic update, local write, cache   |
