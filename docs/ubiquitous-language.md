...
| Term                              | Definition                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Aliases to avoid                    |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| **TMDB** (updated)                | The Movie Database — the **one outside source the app consults**, and only through **Enrichment** (log 23): a **TMDB key** in Settings, a **Sync**, `tmdb_id` on every **Movie** and **Series** it settles. Still not consulted by **Bulk import** (`13-bulk-import.md` Q18).                                                                                                                                                                                                   | the API, metadata service           |
| **Library root** (updated)        | The top folder holding the family's own **Source folders** — **not owned by the app** and never served from. The **Movie form** picks files from it through a browser picker; **Bulk import** is handed its path, walks it, copies out of it, and — since log 23 — **remembers it** (`settings.library-root`). Written to by nothing but the two **Write targets**, and never over a file that exists.                                                                          | media folder, source folder         |
...
| **Optimistic save** (updated)     | The bargain an edit keeps with the server: show the new value at once, take the route's echo over what was assumed, put back what the edit cost if the save is refused. Written twice, for two shapes. `useOptimisticSave` reverts by negating a flag and addresses a **Movie** by id inside a list — the two browse screens' **Favorite** hearts, reaching every **Home section** the **Movie** has a card in. `useOptimisticEdit`, in `hooks/`, is told what to put back rather than deriving it, and edits the one record a page is holding — all three of the **Movie detail page**'s writes, the watched tick, its own heart and the **Rating picker**; the **Series page**'s heart; and the **Season page**'s episode box and season mark. A **Rating** is why the second one exists: eleven values plus an absence, where `!value` can express none of it. | optimistic update, local write, cache   |
...
| **Source folder** (updated)  | One film's own directory under the **Library root** — found by the scan rule _a folder holding a video file is a Source folder; one holding none is descended_. Recorded on the **Movie** (and a **Show folder** on its **Series**) as `source_folder`, **relative to the Library root**, by the import that added or recognised it. Never written to but by a **Write target**. Distinct from the **Movie folder**, which is the app's copy of it.                                                                                   | movie folder (wrong), film folder, the folder |
...
| **Enrichment** (updated)     | Filling what the **Sheet** left blank from **TMDB**, over the library already imported — its own initiative (`enrichment`, design log 23) and its own screen, `/enrich`. Keyed by title + year; never a second scanner, never the household **Rating**. See _Enrichment_ below.                                                                                                                                                                                                                                                       | TMDB import, metadata fetch, lookup           |
...
## The Snackbar system (new)

The app's one channel for something that happened away from where the family is
...
| Term                    | Definition                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Aliases to avoid                                |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| **Back rule** (updated) | The one way any screen is left — `useGoBack(fallback)` in `src/hooks/`: a **History step**, or the screen's **Landing** when there is no history behind it. In a line: _arriving pushes, leaving steps_. Every way onto a screen is a push; every way off it that means "done here" — Back, Escape, a save, a Skip, a Delete — is a step. No screen has a rule of its own; moving _within_ a screen is the third shape, a **Sideways move**.                                                                                                                                                       | back logic, exit handling, navigation rule      |
...
| **Landing** (updated)   | A screen's own destination for the no-history case — `location.key === 'default'`, a deep link or a reload — pushed so the screen it lands on has a Back of its own. The library by default; the player's is its **Movie detail page**, Import's is Settings, the **Movie form**'s is where its **Form context** came from: the **Review step** in **Import context**, the movie for an edit, Settings for an add; the **Series page**'s is the **Series tab**, the **Season page**'s its **Series page**, an **Episode**'s player its **Season page**. Read off the URL, never off fetched state. | fallback route, default route, home for, origin |
...
The second kind of thing on the shelves, its own initiative (`series`, design
log 22), step 5 of the build order: a **Series** tab beside Movies, a
**Series page**, a **Season page**, **Episode** watch state, and the importer
and player learning what an **Episode** is. Everything additive — the
**Movie** flow is untouched.

| Term                            | Definition                                                                                                                                                                                                                                                                                                                                                                                             | Aliases to avoid                              |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------- |
| **Series** (new)                | A TV show in the library — one row in `series`, one **Poster card** on the **Series tab**, one **Series page**. Carries a title, a **Year range**, a creator, cast, synopsis, genres, a household **Rating** and a **Favorite**, and no watch state of its own.                                                                                                                                        | show (in code), TV show, programme, title     |
| **Episode** (new)               | One file of a **Series**, one row in `episodes`, numbered by **Season** and episode — the unit that is played, resumed and marked **Watched**, with exactly the movie's watch trio (`watched`, resume position, `last_watched_at`).                                                                                                                                                                    | ep, chapter, part                             |
| **Season** (new)                | The **Episodes** of a **Series** that share a `season_number` — a number, not a row: there is no `seasons` table, and nothing about a season is stored.                                                                                                                                                                                                                                                | series (for a season), volume, part           |
| **Year range** (new)            | A **Series**' `year` and `end_year`, drawn in the prototype's three shapes: `2022` (they are equal), `2019–2023`, and `2021–` (`end_year` null, still running). Read from the **Sheet**'s _Year_ cell; a lone year is a finished run.                                                                                                                                                                  | airing years, run, dates                      |
| **Series tab** (new)            | The browse home at `?tab=series`, chosen by the **Library tabs**: an **Episode continue card** row over _All series_, a count line and a **Library grid** of **Poster cards**. The header's search, genre, rating and sort all apply to it.                                                                                                                                                            | TV page, shows page, series section           |
| **Library tabs** (new)          | The Movies / Series segmented control first in the browse header — two **Controls** with `aria-pressed`, writing `tab` as a `replace`. Switching clears the search and keeps the filters.                                                                                                                                                                                                              | tab bar, toggle, switcher, segmented tabs     |
| **Series page** (new)           | `/series/:id` — the movie page's hero shape for a **Series**: poster, **Year range**, season and episode count, read-only stars, genres, _Resume S02E04_, the heart, the progress line, synopsis, _Created by_ / _Starring_, over the **Season cards**. No ⋯ menu, no rating picker, no watched toggle. Its Back is `page.SeriesPage`'s 44px glass circle, icon-only — not the movie page's text pill. | series detail, show page                      |
| **Season page** (new)           | `/series/:id/season/:n` — one **Season**'s **Episode rows** under the series title, _Resume E04_, the count line, _Mark season watched_, and the _Other seasons_ pills. Its **Landing** is the **Series page**.                                                                                                                                                                                        | episode list, season detail                   |
| **Season card** (new)           | `components/SeasonCard/` — the 2:3 **Card** on the **Series page**: `S02` over the series' gradient, a watched badge when the **Season** is complete, a bar when part-watched, and "8 episodes" or "3 of 8 watched" under _Season 2_.                                                                                                                                                                  | season tile, season poster                    |
| **Episode row** (new)           | `components/EpisodeRow/` — one **Episode** on the **Season page**: 16:9 gradient thumbnail with a hover play glyph and resume bar, `S02E04` and the title, the air date, the **Resume label**, and a watched box that only marks. A **Card**; its box a **Control**.                                                                                                                                   | episode card, episode item, list row          |
| **Next episode** (new)          | `nextEpisodeOf` — the first part-watched **Episode**, else the first unwatched, else the first; answered once, on the server, for a **Series** and for each **Season**. What _Resume_ / _Play_ on both pages plays; its first arm alone is what the **Episode continue card** shows.                                                                                                                   | up next (for this), resume target, current ep |
| **Episode continue card** (new) | A **Continue card** on the **Series tab** — one per **Series** with a part-watched **Episode**, reading `Harbor & Vine · S02E04` over the **Resume label**, ordered by last watched; it opens the player, not a page.                                                                                                                                                                                  | series continue card, resume card             |
| **Playable** (updated)          | What the **Player** is handed, and the only address it or its hooks and wire take: `{ kind: 'movie' \| 'episode'; id }`, choosing which of two parallel wires (`/api/movies/:id/*`, `/api/episodes/:id/*`) its reads and writes run over. One player, one set of rules, two kinds of file.                                                                                                             | media item, video, source, track              |
| **Up next card** (new)          | `features/player/UpNextCard/` — the card in an **Episode**'s last 15s: the **Next episode** in series order, _Up next · in 12s_ counting down to the end of the file, _Play now_ and _Cancel_. Cancel means the next one does not play.                                                                                                                                                                | autoplay prompt, next-up, countdown           |
| **Sideways move** (new)         | The **Back rule**'s third shape: a `replace` that swaps what one screen shows without adding a step — the _Other seasons_ pills and the **Up next card**. Back after either still reaches the screen before it.                                                                                                                                                                                        | replace navigation, swap, in-place nav        |
| **Show folder** (new)           | A **Source folder** grouping that is a **Series**: the parent of one or more **Season folders**, or a folder whose own videos carry an **Episode tag** (loose episodes). Found after the walk by `groupShows`; everything else is a film.                                                                                                                                                              | series folder (for the source), TV folder     |
| **Season folder** (new)         | A **Source folder** named `Season 01`, `Season 1` or `S01` inside a **Show folder**; its number wins over the filename's.                                                                                                                                                                                                                                                                              | season dir                                    |
| **Episode tag** (updated)       | The `S01E03` or `1x03` in an episode's filename — `episodeTag`, in `media/`, reads the season, the episode (the first of a multi-episode file) and the title after it, and `spellEpisodeTag` writes it back as `S01E03`. The only source of an episode number. On the client, `formatEpisodeTag` is its one spelling — `S02E04`, `S02` or `E04`.                                                       | episode code, SxxEyy, marker                  |
| **Series folder** (new)         | One **Series**' directory under the **Managed media directory**, reserved like a **Movie folder** in the same namespace (`harbor-and-vine-2019`), with the **Episodes** under `season-01/` and the art at its top.                                                                                                                                                                                     | show dir, series bucket                       |
| **Unplaced** (new)              | The `unplaced` hard **Problem**: an episode video in a **Show folder** that no rule can number, or a second file claiming a taken number. Its reason names the fix — rename, import again — and its row has _Skip_ and no _Resolve_.                                                                                                                                                                   | unparsed, orphan episode, unknown episode     |

## Enrichment (new)

The one feature that goes online — its own initiative (`enrichment`, design
log 23), step 6 of the build order: a **Network group** in Settings, a **Sync**
over the library already imported, and a review of what it could not settle.
Movies and series both.

| Term                             | Definition                                                                                                                                                                                                                                                                                                                 | Aliases to avoid                           |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| **Network group** (new)          | The fifth **Settings group**, between Playback and Storage: _The Movie Database (TMDB)_, its status pill, the **TMDB key** field with _Test connection_, and the _Sync metadata & posters_ row. `features/settings/NetworkSection/`.                                                                                       | Internet settings, TMDB settings           |
| **TMDB key** (new)               | The maintainer's TMDB credential — a v3 API key or a v4 Read Access Token, told apart by `tmdbAuth` — kept in `settings.tmdb-api-key` and stored **only after TMDB accepted it**: _Test connection_ is the save. **Connected** means one is stored and the field still holds it.                                           | API key (alone), token, password           |
| **Sync** (new)                   | One **Enrichment** pass from _Start sync_ (or _Fetch details_) to _Done_ — the verb on every surface: _Sync with TMDB_, _Sync metadata & posters_, _Sync again_. Say **Enrichment** for the feature, **Sync** for doing it once.                                                                                           | fetch (as a noun), refresh, update         |
| **Current enrichment run** (new) | The one **Sync** the server holds in memory — `GET /api/enrichment/current` — `running` then `review`, polled at 500 ms and re-attachable, the **Current run**'s shape. A start while one exists is `409`; _Stop_ drops it, keeping everything already written.                                                            | job, sync session                          |
| **Enrichment scope** (new)       | Which titles a **Sync** covers: `missing` (every title without **Full details**; fills gaps only, never a **Field conflict**), `all` (every movie and series), `single` (one **Movie**, from its ⋯ menu). Chosen on the setup's scope cards.                                                                               | sync mode, target, filter                  |
| **Full details** (new)           | A **Movie** or **Series** with both a synopsis and a **Poster**. The Settings row's "N of M titles have full details"; every other title is what _Only what's missing_ syncs.                                                                                                                                              | complete, enriched, synced                 |
| **Enrichment field** (new)       | One of the ten chips under _Fields to fill_ — Synopsis, Poster, Backdrop, Runtime, Year, Genres, Director, Cast, Original title, TMDB score. A chip off is a column never written this **Sync**. A series' Director is its creator; an episode's Poster is its **Still**.                                                  | column, attribute, metadata item           |
| **TMDB score** (new)             | TMDB's `vote_average`, 0–10, stored as `tmdb_score` beside the household **Rating** and never over it. Stored, not drawn anywhere.                                                                                                                                                                                         | TMDB rating, rating (for this), score      |
| **Still** (new)                  | An **Episode**'s 16:9 frame from TMDB, stored as `still_path` beside the episode and drawn in `EpisodeRow`'s thumbnail over the **Gradient fallback**.                                                                                                                                                                     | thumbnail (for the file), screenshot       |
| **Candidate** (new)              | One TMDB search result offered for a title: its id, title, year, genre, language, poster URL and **Match score**. Drawn in the candidate picker; its poster is the one image the app shows straight from TMDB.                                                                                                             | result, suggestion, option                 |
| **Match score** (new)            | A **Candidate**'s _% match_: 70 for the title (**Title key** equal, else word overlap), 30 for the year (equal; 15 one off). `matchScore`, pure.                                                                                                                                                                           | confidence, similarity, relevance          |
| **Confident** (new)              | A title whose search returned exactly one **Candidate** with an equal **Title key** and, when the title has a year, an equal year — written during the run without a look, the **Match** rule's twin.                                                                                                                      | auto-match, sure match                     |
| **Field conflict** (new)         | A filled field TMDB answers differently — Synopsis, Year, Genres, Director or Cast only, compared trimmed and case-blind, lists as sets. Never raised in the `missing` **Enrichment scope**. Drawn as a _Yours / TMDB_ row, TMDB chosen by default.                                                                        | diff, mismatch, clash                      |
| **Decision** (new)               | A title the **Current enrichment run** could not settle alone, listed under "need your decision": `ambiguous` (several **Candidates** — pick one), `missing` (none — search), `conflict` (**Field conflicts** — _Apply choices_ or _Keep all mine_). Every one has _Skip_, which is **Dismiss**.                           | Problem (the import's), pending row, issue |
...
- A **Rating** belongs to exactly one **Movie**; it is **Unrated** until someone sets it from a **Rating picker** — on the **Movie detail page** or in the **Movie form**. **TMDB** never seeds it: the **TMDB score** is a separate column beside it (log 23).
...
- A **Library root** is never written by a **Delete**, an edit or an import — only the **Managed media directory** is ours to remove from. The one exception is the two **Write targets** of a **Sync**, which only ever add a file that is not there.
...
- A **Sync** runs as at most one **Current enrichment run** at a time; it covers one **Enrichment scope** and writes only its chosen **Enrichment fields**. Every title in it ends **Confident** (written), as a **Decision**, or with no result logged.
- A **Decision** leaves the review exactly once: by a pick, a search then a pick, _Apply choices_, or **Dismiss** (_Skip_, _Keep all mine_).
- A **Movie** or **Series** settled by a **Sync** carries a `tmdb_id`; the next **Sync** fetches it by that id and never searches for it again.
- The household **Rating** and every watch state are outside **Enrichment**: no **Sync** reads or writes them.
- A **Source folder** is read and copied from, never removed, and written to only by a **Write target** adding a `poster.jpg` it lacks; a **Movie folder** is written to and removed, never read from as a source. The two never share a path.
...
- The **Snackbar stack** is app-level, because its callers are not in one place. Its first caller is **Enrichment** (log 23 Q9) — the key test, a Start with no key, a pick, an apply and a search — not the **Software update** flow log 18 expected; none of the six that once refused one is reopened to give it work.
...
- A **Series** has one or more **Episodes**, and every **Episode** belongs to exactly one **Series** and one **Season** of it; `(series, season, episode)` is unique. A **Season** exists exactly as long as one **Episode** carries its number.
- An **Episode** is **Watched**, **In-progress** or unwatched by the movie's own derivation. A **Season** is complete when all its **Episodes** are watched; a **Series** is watched when all its **Episodes** are. Neither is stored.
- A **Series** enters the library only through **Bulk import**, one **Show folder** at a time; a second run over the same folder adds only the **Episodes** the **Series** lacks. There is no form for a **Series**, and no **Delete**.
- A **Show folder** is matched to a **Sheet row** by the movie's **Title key** rule; the row's _Director_ is read as the creator. A **Show folder** no row names still imports, under its folder's name.
...
>
> **Dev:** "We're building the **Snackbar stack** next, and nothing in the app
> raises one. Isn't that the component-with-no-caller thing we refuse?"
> **Maintainer:** "It would be if nobody had said what it's for. The **Update
> offer snackbar** is written down to the word — _Update available_, info,
> **Update now**, persists. What's missing is Electron, and the stack doesn't
> touch Electron. So it goes first."
> **Dev:** "Then do I give it something to do meanwhile? The refused rating
> save reverts silently."
> **Maintainer:** "No. That one refused a snackbar because the prototype draws
> none there, and it still draws none there. Six screens said that. None of
> them reopens."
> **Dev:** "What do I hand `notify`?"
> **Maintainer:** "A **Snackbar notice** — variant, message, an optional title,
> an optional action. Say the whole word: a **Player notice** is the thing in
> the play circle, and I don't want two notices in one glossary."
> **Dev:** "And how long does it stay?"
> **Maintainer:** "One rule. It's an **Actionable snackbar** or it isn't. With
> an action it stays until someone deals with it; without one it's gone at
> five seconds. No duration argument — the second a caller can pass a number,
> that's two rules."
> **Dev:** "If the family press **Update now** on the offer, does it dismiss
> itself?"
...
> **Dev:** "The spec says a `series`, `season`, `episode` trio. What goes in
> `seasons`?"
> **Maintainer:** "Nothing — a **Season** is a number on its **Episodes**.
> The row would only hold counts the episodes already answer."
> **Dev:** "The family finishes S02E04 at 95%. What does _Resume_ on the
> **Series page** play now?"
> **Maintainer:** "The **Next episode**: nothing's part-watched, so the first
> unwatched — S02E05. And the **Episode continue card** goes, because it only
> shows a part-watched one."
> **Dev:** "And in the player, E05 was already coming — the **Up next card**
> was counting down."
> **Maintainer:** "Then it plays, by a **Sideways move**: a replace. Back from
> E05 is the **Season page** they started on, not E04's player. Had they
> pressed _Cancel_, it stops at the end like a film."
> **Dev:** "A file called `Harbor.and.Vine.Bonus.mkv` in `Season 02/`?"
...
- **Snackbar, not toast or banner (new):** the prototype's file is
  ...
  **Snackbar**.
- **"Notice" was about to mean two things (new):** a **Player notice** is the
  message drawn in the big-play circle, and log 17's interface sketch called
  the thing handed to `notify` a `Notice` as well. Resolved in favour of both
  keeping their qualifier: **Player notice** and **Snackbar notice**
  (`SnackbarNotice`), never a bare "notice" for either. The prototype's
  `pushSnack` / "snack" is not adopted — it names no rung and reads as slang
  beside every other term here.
- **A system is not a component with no caller (new):** log 17 Q2 refused a
  Snackbar built alone, and log 18 Q2 builds it alone anyway. The distinction
  is that a _speculative_ component is one nothing has specified, and by then
  the **Update offer snackbar**, its variant, its copy and its persistence were
...
- **"Trio" against two tables (new):** COMPONENT-SPEC §5aa and CLAUDE.md's
  Series entry say "a `series`, `season`, `episode` trio". Log 22 Q3 built no
  `seasons` table — a **Season** holds nothing but its number. The spec's line
  was a data note for implementers, not a surface; read "trio" as the three
  words, not three tables.
- **Continue Watching on the Series tab was described two ways (new):** the
  prototype's code lists every part-watched **Episode**; §5aa says the row
  "follows `nextEpisodeOf`". Log 22 Q29 draws one **Episode continue card**
  per **Series**, its earliest part-watched **Episode** — the **Next
  episode**'s first arm — which is the same row on the prototype's data.
- **`EpisodeRow`'s own hover against the contract (new):**
  `mol.EpisodeRow.dc.html` lifts 2px, settles to 0 and recolours its fill to
  `surface-2`; §2a lists `EpisodeRow` as a **Card** — 4px, 1px, no fill.
  Log 22 Q38 builds the contract, as log 21 Q2 promised. The file is stale on
  this, not the table.
- **`PosterCardMovie` holds a Series (new):** the **Series tab**'s grid feeds
  `PosterCard` a `PosterCardMovie` built from a **Series**, as the
  prototype's `seriesView` does. The name is now narrower than what it
  carries. The series refactor (201) left it, and `ContinueRow`'s and
  `LibraryGrid`'s `movies` with it, flagged for a round of their own: the
  rename touches the whole movie browse surface.
- **The Back rule's "no third shape" (new):** log 20 wrote that "one rule
  with a third shape is two rules", when it kept the **Fresh homes** as
  pushes. The **Sideways move** is a third shape all the same, named rather
  than hidden: arriving pushes, leaving steps, moving within a screen
  replaces. Two callers — _Other seasons_ and the **Up next card** — and a
  third would need the same argument.
- **The prototype's _Cancel_ auto-played anyway (new):** the container's timer
  plays the next episode at the end whether or not the **Up next card** was
  cancelled. That is the simulation; log 22 Q45 makes _Cancel_ cancel.
- **_Resolve_ is not on every Problem any more (new):** the **Review step**
  drew _Resolve_ and _Skip_ on every row. **Unplaced** has _Skip_ alone,
  because no form can place an **Episode**. Say **Dismiss** for what its Skip
  does, as for every other kind.
- **No series form, no series in Export (new):** a **Series** is added and
  corrected only by **Bulk import** — there is no **Movie form** for one, no
  delete, and the **Export file** holds films only. Log 22 Q2 and its
  Trade-offs flag both for a prototype amendment; the log is a snapshot and is
  not edited, so the flag lives here and in the journal until the prototype
  draws them.
- **"Series folder" and "Show folder" (new):** a **Show folder** is the
  family's, under the **Library root**; a **Series folder** is the app's,
  under the **Managed media directory** — the **Source folder** / **Movie
  folder** split, one level up. Never say "series folder" for the family's.
- **"Problem" and "Decision" (new):** the import's review and the sync's
  review look alike and are siblings by design, but a **Problem** is a film
  not yet in the library (or wanting a look), settled on the **Movie form**,
  and a **Decision** is a title already in it waiting on TMDB. Never call a
  **Decision** a Problem, and never draw _Resolve_ on one.
- **"Missing" twice (new):** `missing` is an **Enrichment scope** (titles
  without **Full details**) and a **Decision** kind (no **Candidate**). In
  prose say _Only what's missing_ for the first and "no result" for the second.
- **The Library root is written to now (new):** logs 11–14 and 22 say the
  **Library root** is never written. Log 23 narrows that for the two **Write
  targets** only; the older logs are snapshots and are not edited.
- **"…this folder name" (new):** the prototype's missing row reads "Nothing on
  TMDB matched this folder name"; a **Sync** searches titles, never folders.
  Log 23 Q27 amends it to "this title" and flags the prototype.
