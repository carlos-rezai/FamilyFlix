...
| **Superseded file** (new) | The **Stored file** a **File slot** held before an edit replaced it. It is unlinked after the row commits — the only deletion of media in the app, one file at a time, matched by **Stored path** rather than by folder. An emptied slot or a detached **Subtitle** authorises no deletion: those are record changes, and the bytes stay.                                                                                                                  | old file, orphan, stale file, leftover     |
